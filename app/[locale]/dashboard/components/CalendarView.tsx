...
const PLATFORMS = [
  { id: "Post Facebook", labelKey: "facebook", icon: <FaFacebook size={14} />, color: "rgba(255,255,255,0.7)" },
  { id: "Post Instagram", labelKey: "instagram", icon: <FaInstagram size={14} />, color: "rgba(255,255,255,0.7)" },
  { id: "Post LinkedIn", labelKey: "linkedin", icon: <FaLinkedin size={14} />, color: "rgba(255,255,255,0.7)" },
  { id: "Email newsletter", labelKey: "email", icon: <MdEmail size={14} />, color: "rgba(255,255,255,0.7)" },
  { id: "Reclamă Meta Ads", labelKey: "metaAds", icon: <FaMeta size={14} />, color: "rgba(255,255,255,0.7)" },
] as const;
...
export default function CalendarView({ onCreditsChange }: Props) {
  ...
  const platforms = PLATFORMS.map((p) => ({ ...p, label: t(`platforms.${p.labelKey}`) }));
  ...
  return (
    <div style={{ fontFamily: "var(--font-geist-sans)", width: "100%" }}>
      ...
      <div style={{ marginBottom: "20px" }}>
        <h1 style={{ fontSize: "22px", fontWeight: 800, color: "#fff", margin: "0 0 4px", letterSpacing: "-0.02em" }}>{t("title")}</h1>
        <p style={{ fontSize: "14px", color: "rgba(255,255,255,0.4)", margin: 0 }}>{t("subtitle")}</p>
      </div>
      ...
      <div style={{ marginBottom: "18px", width: "100%" }}>
        ...
        <p style={{ fontSize: "11px", color: "rgba(255,255,255,0.2)", margin: "6px 0 0" }}>
          {t("creditsInfo", { count: postCount, singular: postCount === 1 ? t("post") : t("posts") })}
        </p>
      </div>
      ...
      <div style={{ display: "flex", gap: "8px", marginBottom: "16px", width: "100%", flexWrap: "wrap" }}>
        ...
        >
          {generatingPlan ? (
            ...
            <span style={{ display: "flex", alignItems: "center", gap: "6px" }}>
              {t("generatePosts", { count: postCount, label: postCount === 1 ? t("post") : t("posts") })}
              <span style={{ color: platformInfo.color, display: "flex", alignItems: "center" }}>{platformInfo.icon}</span>
              {platformInfo.label}
            </span>
          )}
        ...
        {generatedSlots.length > 0 && (
          ...
          >
            ...
            ) : emailSent ? t("emailSent") : t("sendPlan", { count: generatedSlots.length })}
          </button>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div style={{
          ...
          <p style={{ color: "rgba(255,255,255,0.5)", fontSize: "14px", fontWeight: 600, marginBottom: "6px" }}>
            {t("noPosts", { platform: platformInfo.label })}
          </p>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "2px" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "6px", minHeight: "36px" }}>
                  ...
                  {daySlots.length === 0 && (
                    <div style={{
                      ...
                      {isDragTarget && (
                        <span style={{ fontSize: "11px", color: "rgba(86,219,132,0.5)" }}>{t("moveHere")}</span>
                      )}
                    </div>
                  )}
                </div>
          ...
          <div style={{ display: "flex", gap: "16px", marginTop: "8px", paddingLeft: "8px", flexWrap: "wrap", alignItems: "center" }}>
            {[
              { color: "rgba(251,146,60,0.5)", label: t("statuses.suggested") },
              { color: "rgba(86,219,132,0.4)", label: t("statuses.approved") },
              { color: "rgba(86,219,132,0.9)", label: t("statuses.generated") },
            ].map(({ color, label }) => (
