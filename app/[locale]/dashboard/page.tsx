...
const CONTENT_TYPE_KEYS = [
  { v: "Post Facebook", key: "facebookPost" },
  { v: "Post Instagram", key: "instagramPost" },
  { v: "Post LinkedIn", key: "linkedinPost" },
  { v: "Email newsletter", key: "emailNewsletter" },
  { v: "Reclamă Meta Ads", key: "metaAd" },
] as const;

const OBJECTIVE_KEYS = [
  { v: "Vânzare", key: "sale" },
  { v: "Awareness", key: "awareness" },
  { v: "Engagement", key: "engagement" },
  { v: "Promovare ofertă specială", key: "specialOffer" },
] as const;
...
export default function DashboardPage() {
  ...
  const CONTENT_TYPES = CONTENT_TYPE_KEYS.map((item) => ({
    v: item.v,
    l: t(`contentTypes.${item.key}`),
  }));

  const OBJECTIVES = OBJECTIVE_KEYS.map((item) => ({
    v: item.v,
    l: t(`objectives.${item.key}`),
  }));

  const [contentType, setContentType] = useState<string>(CONTENT_TYPE_KEYS[0].v);
  const [objective, setObjective] = useState<string>(OBJECTIVE_KEYS[0].v);
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : t("noCredits.title"));
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        <div style={{
          ...
          {(["generator", "calendar", "vision", "score", "hooks"] as const).map((tab) => (
            ...
            >
              {tab === "generator" ? t("tabs.generator")
                : tab === "calendar" ? t("tabs.calendar")
                : tab === "vision" ? t("tabs.vision")
                : tab === "score" ? t("tabs.score")
                : t("tabs.hooks")}
            </button>
          ))}
        </div>
      ...
      {error === "no_credits" && (
        ...
        >
          <div>
            <p className="text-[14px] font-semibold text-orange-300 mb-1">{t("noCredits.title")}</p>
            <p className="text-[13px] text-white/50 mb-3">{t("noCredits.body")}</p>
            <Link
              href="/pricing"
      ...
      >
        ...
        <div className="mt-4" data-tour="context-field">
          <label className="block text-[11px] font-semibold uppercase tracking-wider text-white/35 mb-2">
            {t("form.context")} <span className="normal-case font-normal text-white/20">{t("form.contextOptional")}</span>
          </label>
          <textarea
            value={context}
        ...
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                {emailSent ? t("result.sent") : sendingEmail ? t("result.sending") : t("result.sendEmail")}
              </button>
            </div>
          </div>
          ...
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "12px" }}>
              <span style={{ fontSize: "13px", fontWeight: 700, color: "rgba(255,255,255,0.7)" }}>
                {t("result.generateImage")}
              </span>
              <span style={{
                fontSize: "10px", fontWeight: 700, padding: "2px 7px", borderRadius: "20px",
                background: "rgba(129,140,248,0.12)", color: "#818cf8",
              }}>
                {t("result.imageCredits")}
              </span>
            </div>
            ...
            <div style={{ display: "flex", gap: "6px", marginBottom: "12px" }}>
              {([
                { id: "1:1", label: t("imageFormats.square"), sub: "1:1", hint: "Feed" },
                { id: "4:5", label: t("imageFormats.portrait"), sub: "4:5", hint: "Story" },
                { id: "16:9", label: t("imageFormats.landscape"), sub: "16:9", hint: "Cover" },
            ...
            >
              {generatingImage ? (
                <>
                  ...
                  {t("result.generatingImage")}
                </>
              ) : (
                <>{t("result.generateImageCta")}</>
              )}
            </button>
              ...
              return (
                ...
                >
                  ...
                  {item.result.length > 100 && (
                    ...
                    >
                      {isExpanded ? t("history.collapse") : t("history.showMore")}
                    </button>
                  )}
                </div>
              );
